...
/// Main render function
pub fn render(f: &mut Frame, app: &App, show_master_prompt: bool, master_input: &str, custom_prompt: Option<&str>) {
    let size = f.area();
...
fn render_master_password_prompt(f: &mut Frame, input: &str, size: Rect, custom_prompt: Option<&str>) {
    let area = centered_rect(50, 30, size);
